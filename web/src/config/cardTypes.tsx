...
/* ---- Greek card types (generated by the Sprint 5 pipeline; editors are
        minimal because these cards are born from the corpus, not typed in).
        Greek-specific logic stays in registry entries per Divergence Rule 6. */

type VocabGkPayload = {
  lemma: string
  gloss: string
  rank: number
  examples: { token_id: number; ref: string; surface: string }[]
}

const vocabGk: CardTypeSpec = {
  key: 'vocab_gk',
  label: 'Greek vocab',
  fields: [
    { name: 'lemma', label: 'Lemma', kind: 'text', required: true },
    { name: 'gloss', label: 'Gloss', kind: 'text', required: true },
  ],
  fromFields(values) {
    const lemma = (values.lemma ?? '').trim()
    const gloss = (values.gloss ?? '').trim()
    if (!lemma || !gloss) throw new Error('Lemma and gloss are required')
    return { lemma, gloss, rank: 1, examples: [] }
  },
  toFields(payload) {
    const p = this.sanitize(payload) as VocabGkPayload
    return { lemma: p.lemma, gloss: p.gloss }
  },
  sanitize(raw) {
    return safePayload<VocabGkPayload>(
      { defaults: { lemma: '(missing lemma)', gloss: '(missing gloss)', rank: 0, examples: [] } },
      raw,
    )
  },
  variantCount: () => 1,
  renderFront(payload) {
    ...
    return (
      <div>
        <div className="greek vocab-lemma">{p.lemma}</div>
        {p.rank > 0 ? <span className="tag tag-quiet">freq #{p.rank}</span> : null}
      </div>
    )
  },
  renderBack(payload) {
    ...
    return (
      <div>
        <div className="card-face-text">{p.gloss}</div>
        {p.examples.map((ex) => (
          <div key={ex.token_id} className="muted">
            <span className="greek-inline">{ex.surface}</span> — {ex.ref.replaceAll('.', ' ')}
          </div>
        ))}
      </div>
    )
  ...
  summarize(payload) {
    const p = this.sanitize(payload) as VocabGkPayload
    return `${p.lemma} — ${p.gloss}`
  },
}

type ParsingPayload = { surface: string; ref: string; lemma: string; parse: string; gloss: string }

const parsing: CardTypeSpec = {
  key: 'parsing',
  label: 'Greek parsing',
  fields: [
    { name: 'surface', label: 'Form', kind: 'text', required: true },
    { name: 'ref', label: 'Reference', kind: 'text', required: true },
    { name: 'lemma', label: 'Lemma', kind: 'text', required: true },
    { name: 'parse', label: 'Parse code', kind: 'text', required: true },
    { name: 'gloss', label: 'In-context gloss', kind: 'text' },
  ],
  fromFields(values) {
    const surface = (values.surface ?? '').trim()
    const ref = (values.ref ?? '').trim()
    const lemma = (values.lemma ?? '').trim()
    const parse = (values.parse ?? '').trim()
    if (!surface || !ref || !lemma || !parse)
      throw new Error('Form, reference, lemma, and parse code are required')
    return { surface, ref, lemma, parse, gloss: (values.gloss ?? '').trim() }
  },
  toFields(payload) {
    const p = this.sanitize(payload) as ParsingPayload
    return { surface: p.surface, ref: p.ref, lemma: p.lemma, parse: p.parse, gloss: p.gloss }
  },
  sanitize(raw) {
    return safePayload<ParsingPayload>(
      {
        defaults: {
          surface: '(missing form)',
          ref: '',
          lemma: '(missing lemma)',
          parse: '',
          gloss: '',
  ...
  renderFront(payload) {
    ...
    return (
      <div>
        <div className="greek vocab-lemma">{p.surface}</div>
        <div className="muted">parse this form · {p.ref.replaceAll('.', ' ')}</div>
      </div>
    )
  },
  renderBack(payload) {
    ...
    return (
      <div>
        <div className="card-face-text">{decodeParse(p.parse)}</div>
        <div className="muted">
          <span className="greek-inline">{p.lemma}</span>
          {p.gloss ? ` — "${p.gloss}"` : ''} · <code>{p.parse}</code>
        </div>
      </div>
    )
  },
  summarize(payload) {
    const p = this.sanitize(payload) as ParsingPayload
    return `${p.surface} (${p.ref}) — ${p.parse}`
  },
}

export const CARD_TYPES: Record<string, CardTypeSpec> = {
  basic,
  cloze,
  verse,
  vocab_gk: vocabGk,
